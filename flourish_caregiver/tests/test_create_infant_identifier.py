...
class TestInfantSubjectIdentifier(TestCase):
    ...
    def test_infant_subject_identifier_2_children(self):
        ...
        child_1_dataset = mommy.make_recipe(
            'flourish_child.childdataset',
            dob=get_utcnow() - relativedelta(years=4, months=9),
            twin_triplet = True,
            **self.child_dataset_options)
        ...
        child_2_dataset = mommy.make_recipe(
            'flourish_child.childdataset',
        ...
        first_child = mommy.make_recipe(
            ...
            study_child_identifier=child_1_dataset.study_child_identifier,
            child_dob=(get_utcnow() - relativedelta(years=4, months=9)).date(), )

        second_child = mommy.make_recipe(
            'flourish_caregiver.caregiverchildconsent',
            subject_consent=subject_consent,
            study_child_identifier=child_2_dataset.study_child_identifier,
            identity='234513181',
            confirm_identity='234513181',
            child_dob=(get_utcnow() - relativedelta(years=5, months=9)).date(), )

        self.assertTrue(first_child.subject_identifier.endswith('25'))
        self.assertTrue(second_child.subject_identifier.endswith('35'))
