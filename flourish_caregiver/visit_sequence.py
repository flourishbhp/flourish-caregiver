...
class VisitSequence(VisitSequence):
    def __init__(self, appointment=None):
        self.initialize_appointment_related_properties(appointment)

    def initialize_appointment_related_properties(self, appointment):
        self.appointment = appointment
        self.appointment_model_cls = appointment.__class__
        ...
        prev_visit = appointment.schedule.visits.previous(self.visit_code)
        self.previous_appointment = get_previous_by_appt_datetime(self.appointment)

        try:
            self.previous_visit_code = getattr(
                self.previous_appointment, 'visit_code', None) or prev_visit.code
    ...
    def get_previous_appointment(self):
        return self.appointment_model_cls.objects.filter(
            ...
            visit_code=self.previous_visit_code).order_by('-visit_code_sequence').first()
